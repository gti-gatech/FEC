...
def get_energy_consumption_values(cycle_name, number_of_passengers, cycle_location,
...
#    energy_sum_parallel *= 100
    ...
    cycle_observed_work_sum = binned_data_df_series.CycleObservedWork.sum()
    total_entries = binned_data_df.CycleBinCounts.sum()
    energy_sum_battery_electric = cycle_observed_work_sum * \
            hours_in_operation/(total_entries/3600)/ c.INVERTER_EFFICIENCY/ c.MOTOR_EFFICIENCY / c.BATTERY_EFFICIENCY
    
    energy_sum_fuel_cell_electric = cycle_observed_work_sum * \
